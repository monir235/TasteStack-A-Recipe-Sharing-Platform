    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6">
            <div className="flex items-center">
              <svg className="h-6 w-6 text-red-500 dark:text-red-400 mr-3" fill="currentColor" viewBox="0 0 20 20">
        ...
        {recipe && !loading && !error && (
          <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl p-8 border border-white/40 dark:border-gray-700/40 shadow-2xl mb-8">
            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-6">
              <div className="flex-1">
                <h1 className="text-5xl font-bold text-gray-900 dark:text-gray-100 mb-4">{recipe.title}</h1>
                <div className="flex items-center mb-4">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-300">Recipe Creator</p>
                  </div>
                </div>
              </div>
                ...
                <button
                  ...
                  className={`inline-flex items-center px-6 py-3 font-semibold rounded-2xl transition-all transform hover:scale-105 shadow-lg ${
                    isLiked 
                      ...
                      : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-2 border-gray-200 dark:border-gray-600 hover:border-red-300 hover:text-red-500'
                  }`}
                >
                  <svg className={`w-5 h-5 mr-2 ${isLiked ? 'fill-current' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
                  </svg>
                  {isLiked ? t('liked') : t('like')} ({recipe.likes_count || 0})
                </button>
                
                <div className="flex items-center px-6 py-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-2xl border-2 border-yellow-200 dark:border-yellow-800">
                  <span className="text-yellow-500 text-xl">
                    {'★'.repeat(Math.floor(recipe.average_rating || 0))}
                    {'☆'.repeat(5 - Math.floor(recipe.average_rating || 0))}
                  </span>
                  <span className="ml-2 font-semibold text-gray-700 dark:text-gray-200">({recipe.average_rating || 0})</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {recipe && (
          <div className="mb-8">
            <div className="relative h-96 lg:h-[500px] bg-gradient-to-br from-violet-100 to-purple-100 dark:from-violet-800 dark:to-purple-800 rounded-3xl overflow-hidden shadow-2xl">
              {recipe.image ? (
                <img src={recipe.image} alt={recipe.title || 'Recipe image'} className="w-full h-full object-cover" />
              ) : (
                <div className="flex items-center justify-center h-full">
                  <div className="text-center">
                    <svg className="w-24 h-24 text-violet-300 dark:text-violet-500 mx-auto mb-4" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clipRule="evenodd" />
                    </svg>
                    <p className="text-xl font-semibold text-violet-400 dark:text-violet-500">{t('no_image_available')}</p>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="mb-8 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">{t('prep_time')}</p>
          <p className="text-lg font-semibold dark:text-gray-100">{recipe?.prep_time || 'N/A'}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">{t('cook_time')}</p>
          <p className="text-lg font-semibold dark:text-gray-100">{recipe?.cook_time || 'N/A'}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">{t('servings')}</p>
          <p className="text-lg font-semibold dark:text-gray-100">{recipe?.servings || 'N/A'}</p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">Difficulty</p>
          <p className="text-lg font-semibold dark:text-gray-100">{recipe?.difficulty || 'N/A'}</p>
        </div>
      </div>
      ...
      {recipe?.category && recipe.category !== '[]' && recipe.category.trim() && (
        <div className="mb-8">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('categories')}</h2>
          <div className="flex flex-wrap gap-2">
            {[...new Set(recipe.category.split(',').map(cat => cat.trim()).filter(cat => cat))].map((cat, index) => (
              <span key={index} className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200">
                {cat}
              </span>
            ))}
          </div>
      ...
      {/* Recipe Description */}
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('description')}</h2>
        <p className="text-gray-600 dark:text-gray-300">{recipe?.description || t('no_description')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('ingredients')}</h2>
            <ul className="space-y-2">
              {recipe && Array.isArray(recipe.ingredients) && recipe.ingredients.length > 0 ? (
                recipe.ingredients.map((ingredient, index) => (
                  <li key={index} className="flex items-start">
                    <span className="flex-shrink-0 h-5 w-5 text-indigo-600 dark:text-indigo-400">•</span>
                    <span className="ml-2 text-gray-700 dark:text-gray-300">{ingredient}</span>
                  </li>
                ))
              ) : (
                <li className="text-gray-500 dark:text-gray-400">{t('no_ingredients')}</li>
              )}
            </ul>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('instructions')}</h2>
            <ol className="space-y-4">
              {recipe && Array.isArray(recipe.instructions) && recipe.instructions.length > 0 ? (
                recipe.instructions.map((step, index) => (
                  <li key={index} className="flex">
                    <span className="flex-shrink-0 h-8 w-8 rounded-full bg-indigo-100 dark:bg-indigo-800 flex items-center justify-center text-indigo-800 dark:text-indigo-200 font-medium">
                      {index + 1}
                    </span>
                    <span className="ml-4 text-gray-700 dark:text-gray-300">{step}</span>
                  </li>
                ))
              ) : (
                <li className="text-gray-500 dark:text-gray-400">{t('no_instructions')}</li>
              )}
            </ol>
          </div>
        </div>
      ...
      <div className="mt-12 bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('rate_recipe')}</h2>
        <div className="flex items-center">
          <span className="text-gray-700 dark:text-gray-300 mr-4">{t('your_rating')}</span>
          <div className="flex">
            {[1, 2, 3, 4, 5].map((star) => (
              <button
                key={star}
          ...
          <div className="flex items-start">
            ...
            <div className="ml-4 flex-1">
              <textarea
                ...
                className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                rows="3"
              />
                ...
                {editingCommentId === comment.id ? (
                  <div className="mb-2">
                    <textarea
                      ...
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                      rows="3"
                    />
                    <div className="mt-2 flex space-x-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center px-3 py-1 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                      >
                        {t('cancel')}
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    {comment.hidden ? (
                      ...
                      <p className="mt-1 text-gray-700 dark:text-gray-300">{comment.content || t('no_comment_content')}</p>
                    )}
                  </>
                )}
              </div>
